...
module.exports = function acquireDependencies(depList, depContainerObj, cb) {
  ...
  const getAllPromises = function _getAllPromises(key) {
    ...
    return cachedPromises[key] = subDeps.reduce(function (p, k) {
      return p.then(val => getAllPromises(k)).then(function (val) {
        acc[k] = val;
        return acc;
      });

    }, Promise.resolve(null)).then(function (vals) {

      console.log('val val val => ', vals);
      ...
      return new Promise(function (resolve, reject) {
        ...
        if (typeof fn !== 'function') {
          ...
          Promise.resolve(fn.call(undefined)).then(resolve, function (e) {
            reject({
              key: key,
              error: e
            });
